...
class PlotGenerator:
    ...
    # ✅ Added Heatmap plot 
    def _create_heatmap(self, variables: List[str], **kwargs) -> plt.Figure:
        """
        Generate a heatmap for numeric data.
        Automatically disables annotations for large matrices.
        """
        numeric_df = self.data.select_dtypes(include=["number"])
        if numeric_df.empty:
            raise ValueError("No numeric variables available for heatmap.")

        cols = [v for v in variables if v in numeric_df.columns]
        data_to_plot = numeric_df[cols] if cols else numeric_df

        if data_to_plot.shape[1] > 1:
            data_to_plot = data_to_plot.corr()

        num_cells = data_to_plot.shape[0] * data_to_plot.shape[1]
        show_annot = kwargs.get("annot", num_cells <= 400)
